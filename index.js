...
client.login("MzM5NTkwMDI2OTczMjgyMzA0.DFmXZA.N3zeK9n-_OmAppq3Z9NdAWLkjNo");
...
client.on("guildMemberAdd", (member) => {
  ...
  3. If you don't see any of the other channels please be patient contact an admin someone will give you permissions as soon as possible.
  4. Please at least add your first name to your nickname/username by clicking on the cog icon next to your name, you will need to enter your password. To change your nickname you can use the \`.callme\` command followed by the nickname you want to use. Your nickname will only change in this server.
  5. Use @here to notify active user in the current channel. It will help you call the attention of people that may be interested in your post, but __please do not missuse__ @here use it only when absolutely necessary.
	...
	if( currentTime.hour() <= 23 && currentTime.hour() >= 6 ){
		client.guilds.forEach((item, index)=>{
			...
			if ( newUsers[index] && newUsers[index].size > 0 && timesAnHour == 0) {//newUsers.size > 10
				...
				item.channels.get(index).send("Welcome our new CS Raidrz!\n" + userlist);
...
client.on("message", (message) => {
	...
	if( message.system && message.type === 'PINS_ADD' && cityPattern.test(channelName) && message.author.username === 'CS Raidr Bot' ){
		//console.log( message );
		message.delete();
	}
